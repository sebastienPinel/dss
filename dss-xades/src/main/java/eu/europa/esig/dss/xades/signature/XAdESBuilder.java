...
public abstract class XAdESBuilder {
	...
	/**
	 * This variable holds the {@code XPathQueryHolder} which contains all
	 * constants and queries needed to cope with the default signature schema.
	 */
	protected final XPathQueryHolder xPathQueryHolder = new XPathQueryHolder();

	/*
	 * This variable is a reference to the set of parameters relating to the
	 * structure and process of the creation or extension of the electronic
	 * signature.
	 */
	protected XAdESSignatureParameters params;

	/**
	 * This is the variable which represents the root XML document root (with
	 * signature).
	 */
	...
	/**
	 * This method allows to retrieve the {@code CertificatePool} containing
	 * trust anchors.
	 *
	...
	/**
	 * Incorporates the certificate's references as a child of the given parent
	 * node. The first element of the {@code X509Certificate} {@code List} MUST
	...
	protected void incorporateCertificateRef(final Element signingCertificateDom,
			final Set<CertificateToken> certificates) {
		...
		for (final CertificateToken certificate : certificates) {
			...
			DSSXMLUtils.setTextNode(documentDom, x509IssuerNameDom,
					"CN=TEST Banque de France AC Signature forte, OU=0002 572104891, O=Banque de France, C=FR");
