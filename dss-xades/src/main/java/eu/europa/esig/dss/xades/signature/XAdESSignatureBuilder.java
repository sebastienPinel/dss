...
import static eu.europa.esig.dss.xades.XAdESNamespaces.*;
...
/**
 * This class implements all the necessary mechanisms to build each form of the
 * XML signature.
 */
public abstract class XAdESSignatureBuilder extends XAdESBuilder implements SignatureBuilder {
	...
	public static XAdESSignatureBuilder getSignatureBuilder(final XAdESSignatureParameters params,
			final DSSDocument document, final CertificateVerifier certificateVerifier) {
	 ...
	 * @param certificateVerifier
	 */
	public XAdESSignatureBuilder(final XAdESSignatureParameters params, final DSSDocument detachedDocument,
	 ...
	 *
	 * @return A byte array is returned with XML that represents the
	 *         canonicalized <ds:SignedInfo> segment of signature. This data are
	 *         used to define the <ds:SignatureValue> element.
	 * @throws DSSException
	 */
	public byte[] build() throws DSSException {
		...
		if (CollectionUtils.isEmpty(references)) {
			...
			// The SignatureParameters object is updated with the default
			// references.
		...
		/**
		 * We create <ds:Reference> segment only now, because we need first to
		 * define the SignedProperties segment to calculate the digest of
		 * references.
		 */
		...
		byte[] canonicalizedSignedInfo = DSSXMLUtils.canonicalizeSubtree(signedInfoCanonicalizationMethod,
				signedInfoDom);
	...
	private void incorporateCanonicalizationMethod(final Element parentDom,
		...
		final Element canonicalizationMethodDom = DSSXMLUtils.addElement(documentDom, parentDom, XMLNS,
				DS_CANONICALIZATION_METHOD);
		canonicalizationMethodDom.setAttribute(ALGORITHM, signedInfoCanonicalizationMethod);
	}

	/**
	 * This method creates the first reference (this is a reference to the file
	 * to sign) which is specific for each form of signature. Here, the value of
	 * the URI is the name of the file to sign or if the information is not
	 * available the URI will use the default value: "detached-file".
	 */
	...
	/**
	 * Creates KeyInfoType JAXB object. NOTE: when trust anchor baseline profile
	 * policy is defined only the certificates previous to the trust anchor are
	 * included.
	 *
	 * @throws DSSException
	 */
	...
	protected void incorporateReferenceSignedProperties() throws DSSException {
		...
		reference.setAttribute(ID, "xades-" + deterministicId + "-Reference");
		reference.setAttribute(TYPE, "http://uri.etsi.org/01903/#SignedProperties"); // FIXME
		...
		final byte[] canonicalizedBytes = DSSXMLUtils.canonicalizeSubtree(signedPropertiesCanonicalizationMethod,
				signedPropertiesDom);
		if (LOG.isTraceEnabled()) {
		...
		// final Element digestValueDom = documentDom.createElementNS(XMLNS,
		// DS_DIGEST_VALUE);
		// final Text textNode =
		// documentDom.createTextNode("CWWSpgJEUcNFyWHy5VGrTY6+wKuAvkmA6O6I+A5zamI=");
	...
	private void incorporateReference(final DSSReference dssReference) {
		...
		// referenceDom.setAttribute(URI, "#Sig_20160825_152717_MANIFEST");
		...
		// DS_DIGEST_VALUE);
		// final Text textNode =
		// documentDom.createTextNode("G7kXDowRKpf4q4jUvlv20wtlNymZ1/3mIu1fx7tiHAk=");
		// digestValueDom.appendChild(textNode);
	...
	/**
	 * When the user does not want to create its own references (only when
	 * signing one contents) the default one are created.
	 *
	...
	/**
	 * This method performs the reference transformation. Note that for the time
	 * being (4.3.0-RC) only two types of transformation are implemented:
	 * canonicalization & {@code Transforms.TRANSFORM_XPATH} and can be applied
	 * only for {@code SignaturePackaging.ENVELOPED}.
	 *
	 * @param reference
	...
	protected void incorporateSignedProperties() throws DSSException {
		...
		signedPropertiesDom = DSSXMLUtils.addElement(documentDom, qualifyingPropertiesDom, XAdES,
				XADES_SIGNED_PROPERTIES);
		signedPropertiesDom.setAttribute(ID, "xades-" + deterministicId);

		incorporateSignedSignatureProperties();

		unsignedSignaturePropertiesDom = DSSXMLUtils.addElement(documentDom, qualifyingPropertiesDom, XAdES,
	...
	protected void incorporateSignedSignatureProperties() {
		...
		signedSignaturePropertiesDom = DSSXMLUtils.addElement(documentDom, signedPropertiesDom, XAdES,
	...
	private void incorporatePolicy() {
		...
		if ((signaturePolicy != null)) {// && (signaturePolicy.getId() != null))
			...
			final Element signaturePolicyIdentifierDom = DSSXMLUtils.addElement(documentDom,
					signedSignaturePropertiesDom, XAdES, XADES_SIGNATURE_POLICY_IDENTIFIER);

			String signaturePolicyId = signaturePolicy.getId();
			if (StringUtils.isEmpty(signaturePolicyId)) { // implicit
				DSSXMLUtils
						.addElement(documentDom, signaturePolicyIdentifierDom, XAdES, XADES_SIGNATURE_POLICY_IMPLIED);
			} else { // explicit
				final Element signaturePolicyIdDom = DSSXMLUtils.addElement(documentDom, signaturePolicyIdentifierDom,
						XAdES, XADES_SIGNATURE_POLICY_ID);
				final Element sigPolicyIdDom = DSSXMLUtils.addElement(documentDom, signaturePolicyIdDom, XAdES,
						XADES_SIG_POLICY_ID);
				...
				if ((signaturePolicy.getDigestAlgorithm() != null) && (signaturePolicy.getDigestValue() != null)) {
					...
					final Element sigPolicyHashDom = DSSXMLUtils.addElement(documentDom, signaturePolicyIdDom, XAdES,
					...
					DSSXMLUtils.addTextElement(documentDom, sigPolicyHashDom, XMLNS, DS_DIGEST_VALUE,
							bas64EncodedHashValue);
				}

				String spuri = signaturePolicy.getSpuri();
				if (StringUtils.isNotEmpty(spuri)) {
					Element sigPolicyQualifiers = DSSXMLUtils.addElement(documentDom, signaturePolicyIdDom, XAdES,
							XADES_SIGNATURE_POLICY_QUALIFIERS);
					Element sigPolicyQualifier = DSSXMLUtils.addElement(documentDom, sigPolicyQualifiers, XAdES,
	 ...
	 * <SigningCertificate> <Cert> <CertDigest> <ds:DigestMethod
	 * Algorithm="http://www.w3.org/2000/09/xmldsig#sha1"/>
	 * <ds:DigestValue>fj8SJujSXU4fi342bdtiKVbglA0=</ds:DigestValue>
	 * </CertDigest> <IssuerSerial> <ds:X509IssuerName>CN=ICA
	 * A,O=DSS,C=AA</ds:X509IssuerName>
	...
	private void incorporateSigningCertificate() {
		...
		final Element signingCertificateDom = DSSXMLUtils.addElement(documentDom, signedSignaturePropertiesDom, XAdES,
				signingCertificate);
	 ...
	 * ObjectReference="#detached-ref-id"> ......<MimeType>text/plain</MimeType>
	 * ...</DataObjectFormat> </SignedDataObjectProperties>
	 */
	private void incorporateSignedDataObjectProperties() {

		signedDataObjectPropertiesDom = DSSXMLUtils.addElement(documentDom, signedPropertiesDom, XAdES,
		...
		for (final DSSReference reference : references) {
			...
			final Element dataObjectFormatDom = DSSXMLUtils.addElement(documentDom, signedDataObjectPropertiesDom,
					XAdES, XADES_DATA_OBJECT_FORMAT);
			dataObjectFormatDom.setAttribute(OBJECT_REFERENCE, dataObjectFormatObjectReference);

			final Element mimeTypeDom = DSSXMLUtils
					.addElement(documentDom, dataObjectFormatDom, XAdES, XADES_MIME_TYPE);
	 ...
	 * @return the {@code MimeType} of the reference or the default value
	 *         {@code MimeType.BINARY}
	 */
	...
	/**
	 * This method incorporate the content-timestamps within the signature being
	 * created.
	 */
			...
			if (TimestampType.ALL_DATA_OBJECTS_TIMESTAMP.equals(timeStampType)) {
				...
				if (allDataObjectsTimestampDom == null) {
					...
					allDataObjectsTimestampDom = DSSXMLUtils.addElement(documentDom, signedDataObjectPropertiesDom,
							XAdES, XADES_ALL_DATA_OBJECTS_TIME_STAMP);
				}
				...
				if (individualDataObjectsTimestampDom == null) {
					...
					individualDataObjectsTimestampDom = DSSXMLUtils.addElement(documentDom,
							signedDataObjectPropertiesDom, XAdES, XADES_INDIVIDUAL_DATA_OBJECTS_TIME_STAMP);
				}
	...
	/**
	 * This method incorporates the signer claimed roleType into signed
	...
	private void incorporateSignerRole() {
		...
		if ((claimedSignerRoles != null) || (certifiedSignerRoles != null)) {
			...
			if (params.isEn319132()) {
				signerRoleDom = DSSXMLUtils.addElement(documentDom, signedSignaturePropertiesDom, XAdES,
			...
			final Element claimedRolesDom = DSSXMLUtils.addElement(documentDom, signerRoleDom, XAdES,
					XADES_CLAIMED_ROLES);
			...
			if (CollectionUtils.isNotEmpty(certifiedSignerRoles)) {
				...
				if (params.isEn319132()) {
					certifiedRolesDom = DSSXMLUtils.addElement(documentDom, signerRoleDom, XAdES,
							XADES_CERTIFIED_ROLES_V2);
				} else {
					certifiedRolesDom = DSSXMLUtils
							.addElement(documentDom, signerRoleDom, XAdES, XADES_CERTIFIED_ROLES);
				}
	...
	private void incorporateSignatureProductionPlace() {
		...
		if (signatureProductionPlace != null) {
			...
			if (params.isEn319132()) {
				signatureProductionPlaceDom = DSSXMLUtils.addElement(documentDom, signedSignaturePropertiesDom, XAdES,
			...
			if (params.isEn319132()) {
				...
				if (streetAddress != null) {
					DSSXMLUtils.addTextElement(documentDom, signatureProductionPlaceDom, XAdES, XADES_STREET_ADDRESS,
							streetAddress);
				}
			}
			...
			if (stateOrProvince != null) {
				DSSXMLUtils.addTextElement(documentDom, signatureProductionPlaceDom, XAdES, XADES_STATE_OR_PROVINCE,
						stateOrProvince);
			}

			final String postalCode = signatureProductionPlace.getPostalCode();
			if (postalCode != null) {
				DSSXMLUtils.addTextElement(documentDom, signatureProductionPlaceDom, XAdES, XADES_POSTAL_CODE,
						postalCode);
			}
			...
			if (country != null) {
				DSSXMLUtils
						.addTextElement(documentDom, signatureProductionPlaceDom, XAdES, XADES_COUNTRY_NAME, country);
			}
		}
	}

	/**
	 * Below follows the schema definition for this element. <xsd:element
	 * name="CommitmentTypeIndication" type="CommitmentTypeIndicationType"/>
	 * <xsd:complexType name="CommitmentTypeIndicationType"> ...<xsd:sequence>
	 * ...... <xsd:element name="CommitmentTypeId" type="ObjectIdentifierType"/>
	 * ......<xsd:choice> .........<xsd:element name="ObjectReference"
	 * type="xsd:anyURI" maxOccurs="unbounded"/> .........< xsd:element
	 * name="AllSignedDataObjects"/> ......</xsd:choice> ......<xsd:element
	 * name="CommitmentTypeQualifiers" type="CommitmentTypeQualifiersListType"
	 * minOccurs="0"/> ... </xsd:sequence> </xsd:complexType> <xsd:complexType
	...
	private void incorporateCommitmentTypeIndications() {
		...
		if (commitmentTypeIndications != null) {
			...
			final Element commitmentTypeIndicationDom = DSSXMLUtils.addElement(documentDom,
					signedDataObjectPropertiesDom, XAdES, XADES_COMMITMENT_TYPE_INDICATION);

			final Element commitmentTypeIdDom = DSSXMLUtils.addElement(documentDom, commitmentTypeIndicationDom, XAdES,
					XADES_COMMITMENT_TYPE_ID);

			for (final String commitmentTypeIndication : commitmentTypeIndications) {

				DSSXMLUtils.addTextElement(documentDom, commitmentTypeIdDom, XAdES, XADES_IDENTIFIER,
						commitmentTypeIndication);
			}
			// final Element objectReferenceDom =
			// DSSXMLUtils.addElement(documentDom, commitmentTypeIndicationDom,
			// XADES, "ObjectReference");
			// or
			DSSXMLUtils.addElement(documentDom, commitmentTypeIndicationDom, XAdES, XADES_ALL_SIGNED_DATA_OBJECTS);

			// final Element commitmentTypeQualifiersDom =
			// DSSXMLUtils.addElement(documentDom,
			// commitmentTypeIndicationDom, XADES, "CommitmentTypeQualifiers");
		}
	}

	/**
	 * Adds signature value to the signature and returns XML signature
	 * (InMemoryDocument)
	 *
	...
	protected void addTimestamp(final Element timestampElement, final TimestampToken token) {
		...
		// List<TimestampInclude> includes, String canonicalizationMethod,
		// TimestampToken encapsulatedTimestamp) {
		// add includes: URI + referencedData = "true"
		// add canonicalizationMethod: Algorithm
		// add encapsulatedTimestamp: Encoding, Id, while its textContent is the
		// base64 encoding of the data to digest
